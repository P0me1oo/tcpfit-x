...
尚未发布的开发分支可直接运行完整源码目录里的 `bash tcpfit.sh return`。单独复制一个主脚本时，回国模式需要下载对应版本的其他模块。
...
接入命令只下载或调用同版本的测速脚本并传递地址、端口和临时凭据，不嵌入程序。默认从调优端通过校验公钥指纹的 HTTPS 下载；只有 wget 的设备可从仓库版本标签下载接入脚本，再自动安装 curl。这个回退地址需要相应版本已经发布。

一次性随机 token 默认 600 秒有效，可用 `--token-ttl 30..1800` 设置。成功配对即撤销 token，后续会话绑定本次测速端的来源 IP。一个任务只能绑定一个测速端；再次接入会被拒绝，原任务继续。服务器只向已配对的来源 IP 放行临时 iperf3 端口。结束、断线或取消后停止临时服务、撤销凭据并删除本任务的规则。
...
延迟使用 curl 的 `time_connect - time_namelookup`，每次新建连接；有效样本少于三个显示“未取得”，不填零。它不是 ICMP ping，也不包含 TLS 或整个 HTTP 请求耗时。
...
每次任务记录位于 `/var/lib/tcpfit/return/<时间-任务号>/`，包含 `before.json`、每次有效测量的 JSON、`result.json`、`final.json` 及事务恢复记录。原始 iperf3 的临时 cookie 不写入长期记录。认证失败、执行失败、缺少数据的测量不会参与计算，也不会使用公共节点结果替代。
