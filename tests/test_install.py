...
''', encoding="utf-8")
            ...
            def publish(version):
                ...
                for name in ("tcpfit.sh", "install.sh", "tcpfit-return.py", "tcpfit-client.sh"):
                    content = (ROOT / name).read_text(encoding="utf-8").replace("0.7.0", version).encode("utf-8")
            ...
            for name in ("tcpfit.sh", "tcpfit-return.py", "tcpfit-client.sh"):
                self.assertEqual((Path("/usr/local/lib/tcpfit/0.7.0") / name).read_bytes(), (fixture / name).read_bytes())
            resolved = run("bash", "-c", 'source "$1"; return_assets; test "$RETURN_HELPER" = /usr/local/lib/tcpfit/0.7.0/tcpfit-return.py', "test-assets", str(entry))
            ...
            for name in ("tcpfit.sh", "tcpfit-return.py", "tcpfit-client.sh"):
                self.assertEqual((Path("/usr/local/lib/tcpfit/0.7.1") / name).read_bytes(), (fixture / name).read_bytes())
